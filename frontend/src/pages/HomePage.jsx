...
import { useRecoilState } from "recoil";
import postsAtom from "../atoms/postsAtom";
import { useTranslation } from 'react-i18next'; // <-- Added this line for translation

// import SuggestedUsers from "../components/SuggestedUsers"; // Commented out the import

const HomePage = () => {
	const { t } = useTranslation(); // <-- Added this line for translation
	const [posts, setPosts] = useRecoilState(postsAtom);
	const [loading, setLoading] = useState(true);
	const showToast = useShowToast();
	...
	return (
		<Flex gap="10" alignItems={"flex-start"}>
			<Box flex={70}>
				{!loading && posts.length === 0 && (
					<h1>{t('welcome  to pear you have succesfully created an account login in to see the latest brookhouse news.')}</h1> // <-- Translated text
				)}
