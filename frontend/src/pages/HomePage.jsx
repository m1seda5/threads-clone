...
// okay so this is the new homepage with the websocket added 
// pages/HomePage.js
import { Box, Flex, Spinner } from "@chakra-ui/react";
import { useEffect, useState } from "react";
import useShowToast from "../hooks/useShowToast";
...
const HomePage = () => {
  ...
  const [loading, setLoading] = useState(true);
  const showToast = useShowToast();
  const { sendMessage } = useWebSocket('ws://localhost:5000'); // Connect to WebSocket
  ...
  return (
    <Flex gap="10" alignItems={"flex-start"}>
      <Box flex={70}>
        {!loading && posts.length === 0 && (
          <h1>Welcome to Pear, a Brookhouse application.</h1>
        )}
      ...
      >
        {/* Optional: Display Suggested Users */}
      </Box>
